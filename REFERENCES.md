...
| 15 | **lenis-demo** (`abdellahaarab/lenis`, local `Temp/opencode/reference-repos/lenis-demo`) | LERP cursor follower, magnetic-button hook, GSAP-ticker Lenis sync | APPLIED 2026-09-12: `src/Cursor.tsx` + `src/cursor.css` (dot+ring, fine-pointer only), `src/useMagnetic.ts` wired to burger + outro CTA. Zero new deps |
| 16 | **React Bits SplitText** (`DavidHDev/react-bits`, local `Temp/opencode/reference-repos/react-bits`) | Masked char reveal via GSAP SplitText | APPLIED 2026-09-12: hand-rolled in `src/Hero.tsx` (gsap-core only, no `@gsap/react`), `.kv-mask`/`.kv-char` in `chrome.css`, reduced-motion fallback |
| 17 | **Khronos DamagedHelmet** (local `Temp/opencode/reference-repos/gltf-sample-models`) | Canonical PBR lighting test subject | STAGED 2026-09-12 at `reference-mockup/models/DamagedHelmet.glb` (local tests only — CC BY-NC license, NEVER ship in prod bundle) |
| 18 | **curated.design** | Handpicked live-site gallery, filterable by industry/style (Agency, AI, Portfolio, Web3…) | RESEARCH ONLY — never copy designs. Pair with skills `awwwards-website` (archetypes/scoring) + `frontend-design` for art direction |
| 19 | **recent.design** | Inspiration feed: Motion / 3D / Typography categories (liquid-mask-cursor-fx, icon-morph, pinned-scroll, voice-orb) | RESEARCH ONLY. Pair with skills `awwwards-micro-interactions` + `motion-framer` + `gsap-scrolltrigger` for micro-interactions |
| 20 | **21st.dev** | 12k+ React components/templates/shadcn themes; ships as AI prompt (paste → agent rebuilds in your stack) or shadcn CLI. Categories: Shaders, Liquid & metal, Galleries & 3D, Animated heroes | SOURCE via prompt-paste, then adapt by hand (no Tailwind in xeven). Pair with skill `animated-component-libraries` + `landing-page`. Free 2 copies/day |
| 21 | **originkit.dev** | 360+ free animated components (Background 100+, Text 60+, Cursor 21, Gallery 30), GPU-safe + reduced-motion aware, live variable editor, MCP server, `npx originkit add` CLI | SOURCE via copy/MCP, adapt by hand. Direct hits: Cursor + Text + Background categories. Pair with skills `awwwards-micro-interactions` + `lightweight-3d-effects`. Caps: free within daily/weekly limits, some Pro items |
| 22 | **threeui.com** | Copy-ready Three.js components/templates/shaders (208 shader-tagged, 216 threejs-tagged) + MCP. Direct hits: Portal Field (portal animation), Warp Field Hyperspace (space transition), Shader Buttons, landing templates | SOURCE via copy, adapt to R3F. Pair with skills `threejs-webgl` + `shaders` + `react-three-fiber` + the 7-technique playbook. Some Pro items |
