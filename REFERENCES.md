...
- `reference-mockup/jera-ref.jpg` is the dev-only intro geometry lock (white field, black double-chevron) — read for taste, never imported.
- Phase 2 hygiene (2026-09-12): dead modules deleted (`CityScene`, `sound`, trail GLSL), dead CSS removed (preloader/gate/retired intros), `@react-three/drei` uninstalled; `lucide-react`/`valtio`/`clsx` stay installed-but-unimported until a step requires them.
