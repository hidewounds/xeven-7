...
export default function VoidWorld() {
  ...
  useEffect(() => {
    ...
    const failGL = (reason: string) => {
      ...
      window.dispatchEvent(new CustomEvent('xeven:nowebgl'))
    }
    ...
    try {
      if (forceNoGL) throw new Error('forced')
      ...
      failGL(forceNoGL ? 'forced' : 'failed')
      return
    }
    ...
    const onRestored = () => {
      ...
      window.dispatchEvent(new CustomEvent('xeven:webgl'))
      if (reduced) {
        renderer.render(scene, camera)
      } else {
        wake()
